    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<RegisterResponse> RegisterAsync(RegisterRequest request)
        {
            ...
            var user = new User
            {
                ...
                LanguagePreference = _translationService.SupportedLanguages.Contains(request.Language ?? string.Empty)
                    ? request.Language!
                    : "az",
            };
        ...
        public async Task<MessageResponse> ForgotPasswordAsync(ForgotPasswordRequest request, string appBaseUrl)
        {
            ...
            if (user is not null && user.PasswordHash is not null)
            {
                ...
                if (!withinCooldown)
                {
                    ...
                    var language = user.LanguagePreference;
                    await TrySendEmailAsync(
                        user.Email,
                        _translationService.Translate(language, "email.passwordReset.subject"),
                        EmailTemplates.BuildPasswordResetEmail(_translationService, language, user.FirstName, resetLink));
                }
            }
        ...
        private async Task SendVerificationCodeAsync(User user)
        {
            ...
            await TrySendEmailAsync(
                user.Email,
                _translationService.Translate(language, "email.verification.subject"),
                EmailTemplates.BuildVerificationEmail(_translationService, language, user.FirstName, code));
        }
        ...
        public async Task<UserDto> UpdateTimezoneAsync(Guid userId, UpdateTimezoneRequest request)
        {
            var user = await _userRepository.GetByIdAsync(userId)
                ?? throw new NotFoundException("User not found.");
            ...
            if (!string.IsNullOrWhiteSpace(request.PreferredTimezone))
            {
                try
                {
                    // Validates against the IANA tzdb (ICU-backed on Linux/macOS/modern
                    // Windows) without needing to maintain a duplicate allow-list server-side.
                    TimeZoneInfo.FindSystemTimeZoneById(request.PreferredTimezone);
                }
                catch (TimeZoneNotFoundException)
                {
                    throw new BadRequestException($"Unknown time zone: '{request.PreferredTimezone}'.");
                }
                catch (InvalidTimeZoneException)
                {
                    throw new BadRequestException($"Invalid time zone data for: '{request.PreferredTimezone}'.");
                }
            }

            user.PreferredTimezone = string.IsNullOrWhiteSpace(request.PreferredTimezone) ? null : request.PreferredTimezone;
            await _userRepository.UpdateAsync(user);
            return user.ToDto();
        }

        public async Task<UserDto> UpdateLanguageAsync(Guid userId, UpdateLanguageRequest request)
        {
            var user = await _userRepository.GetByIdAsync(userId)
                ?? throw new NotFoundException("User not found.");

            if (!_translationService.SupportedLanguages.Contains(request.Language))
